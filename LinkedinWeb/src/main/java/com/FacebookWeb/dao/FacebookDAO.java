...
public class FacebookDAO implements FacebookDAOInterface {
	...
	public int createProfileDAO(FacebookUser fb, FacebookLogin fbl) {
		int i=0;
//		FacebookPostdata fbp=new FacebookPostdata();
		try {
...
//			  System.out.println("profile");
	...
	public List<FacebookPostdata> postDAO(FacebookLogin fbl) {
		try {
			Session s = sf.openSession();
			Query q = s.createQuery("from com.FacebookWeb.entity.FacebookPostdata f");
			
			/*
			 * List<FacebookPostdata> fl= q.getResultList(); for(FacebookPostdata ff:fl) {
			 * if(ff.getFbuser().equals(fbu.getEmail())) { fbuData=ff; break; } }
			 */
		}
		catch(Exception e) {
			e.printStackTrace();
